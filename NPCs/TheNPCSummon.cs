		...
		public static bool ChestItemSummonCheck(int x, int y, Mod mod)
		{
			...
			if (tileType == TileID.Containers && (tileStyle < 5 || tileStyle > 6))
			{
				for (int i = 0; i < 40; i++)
				{
					...
					{
						if (Main.chest[num].item[i].type == mod.ItemType("WeaponFuel"))
						{
							numberWeaponFuel += Main.chest[num].item[i].stack;
						}
						else
						{
							numberOtherItems++;
			...
			if (numberOtherItems == 0 && numberWeaponFuel == 3)
			{
				...
				int npcToSpawn = mod.NPCType("TheNPC");
